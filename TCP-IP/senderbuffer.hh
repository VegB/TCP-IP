...
#define SENDER_BUFFER_SIZE 1000
...
private:
    ...
    uint32_t _sender_start_pos;
    uint32_t _sender_end_pos;
    uint32_t _last_acked;  // max ACK
    uint32_t _last_acked_cnt;  // how many times '_last_acked' has been received
    
    WritablePacket* ReadOutDataPacket(int);
    WritablePacket* CreateInfoPacket();
    uint32_t SenderBufferRemainSize(uint32_t s, uint32_t e);
